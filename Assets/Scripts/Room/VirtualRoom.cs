...
public class VirtualRoom : MonoBehaviour
{
    ...
    public NavMeshSurface sceneNavMesh;
    bool _roomOpen = false;
    List<PartySceneObject> _roomboxFurnishings = new List<PartySceneObject>();
    List<PartySceneObject> _roomboxWalls = new List<PartySceneObject>();
    [HideInInspector]
    public GameObject roomCenter;
    public bool generateSolidMesh = false;
    public bool generateContinuousMesh = false;
    

    public SceneMesher _sceneMesher;
    [HideInInspector]
    public List<Vector3> _cornerPoints = new List<Vector3>();
    int _roomFloorID;
    int _roomCeilingID;
    // Differing from example here. All calls to info about the virtual room space should be handled here rather than the PartySceneSingleton
    [HideInInspector]
    public float _floorHeight = 0.0f;
    [HideInInspector]
    public float _wallHeight = 1.0f;
    ...
    public void Initialize(OVRSceneAnchor[] sceneAnchors)
    {
        ...
        List<PartySceneObject> anchorsAsWBRO = new List<PartySceneObject>();
        for (int i = 0; i < sceneAnchors.Length; i++)
        {
            ...
            PartySceneObject wbro = instance.GetComponent<PartySceneObject>();
            if (wbro == null)
            {
                Debug.Log("No child of type Party Room Object on anchor: " + instance.name);
                continue;
            }
                ...
                classification.Contains(OVRSceneManager.Classification.DoorFrame))
            {
                ...
                NavMeshModifier wbroModifier = wbro.gameObject.GetComponent<NavMeshModifier>();
                if (wbroModifier == null)
                    wbroModifier = wbro.gameObject.AddComponent<NavMeshModifier>();
                wbroModifier.overrideArea = true;
                ...
                if (classification.Contains(OVRSceneManager.Classification.Floor))
                {
                    ...
                    instance.name = "Virtual Room Floor";
                    wbroModifier.area = 0;
                ...
                else if (classification.Contains(OVRSceneManager.Classification.Ceiling))
                {
                    ...
                    instance.name = "Virtual Room Ceiling";
                    wbroModifier.area = 1;
                ...
                {
                    wbroModifier.area = 1;
                    instance.name = "Virtual Room Door";
                    CreateNavMeshObstacle(wbro, new Vector3(1,1,1));
                }
                ...
                if (classification.Contains(OVRSceneManager.Classification.WallFace))
                {
                    wbro._isWall = true;
                    wbroModifier.area = 1;
                    instance.name = "Virtual Room Wall";
                    CreateNavMeshObstacle(wbro, new Vector3(1,1,0.1f));
                }
                ...
                // Add to navmesh layer
                wbro.gameObject.SetLayerRecursively(navMeshLayer);
            }
                ...
                classification.Contains(OVRSceneManager.Classification.Other))
            {
                ...
                if (bc)
                {
                    NavMeshObstacle obstacle = bc.GetComponent<NavMeshObstacle>();
                    if(obstacle == null)
                        obstacle = bc.gameObject.AddComponent<NavMeshObstacle>();
                ...
                wbroModifier.area = 1;
                wbro.gameObject.SetLayerRecursively(navMeshLayer);
                CreateNavMeshObstacle(wbro, new Vector3(1,1,1),true);
            }
        }

        if (_roomboxWalls.Count < 5)
        {
            Debug.LogError("Not enough walls in the room!");
        }

        _cornerPoints = GetClockwiseFloorOutline(anchorsAsWBRO.ToArray());

        if (generateSolidMesh)
        {
        ...
        if (generateContinuousMesh)
        {
            CreateSpecialEffectMesh(anchorsAsWBRO.ToArray());
        }

        _floorSceneAnchor.transform.eulerAngles = new Vector3(90, 0, 0);
        sceneNavMesh.BuildNavMesh();
    ...
    /// </summary>
    public void CreateSpecialEffectMesh(PartySceneObject[] worldBeyondObjects)
    {
    ...
    /// </summary>
    List<Vector3> GetClockwiseFloorOutline(PartySceneObject[] allFurniture)
    {
        List<Vector3> cornerPoints = new List<Vector3>();
        // New boundary...?
        NativeArray<Vector2> boundary = OVRPlugin.GetSpaceBoundary2D(_floorSceneAnchor.Space, Allocator.Temp);

        if (null == _floorSceneAnchor || !OVRPlugin.GetSpaceBoundary2D(_floorSceneAnchor.Space, boundary))
        {
            // fall back to wall corner method
            List<PartySceneObject> justWalls = new List<PartySceneObject>();
            ...
            {
                Debug.LogError("Not enough walls in floor outline. Data likely corrupt; prompt user to redo room.");
            }
        }
        else
        {
    ...
    /// </summary>
    Vector3 GetNextSplinePoint(ref int thisID, List<PartySceneObject> randomWalls)
    {
        Vector3 nextScale = randomWalls[thisID].GetComponent<PartySceneObject>()._dimensions;
        ...
        for (int i = 0; i < randomWalls.Count; i++)
        {
            ...
            if (i != thisID)
            {
                Vector3 thisWallHalfScale = randomWalls[i].GetComponent<PartySceneObject>()._dimensions * 0.5f;
        ...
        {
            Debug.Log("Error: Room outline is not continuous (missing a wall?)");
        }
    ...
    /// </summary>
    void CreatePolygonMesh(List<PartySceneObject> roomSurfaces, PartySceneObject srObject, bool flipNormal)
    {
        try
        {
    ...
    /// </summary>
    //void CreateWallDebris(PartyRoomObject wallObject)
    //{
    ...
    /// </summary>
    void CreateNavMeshObstacle(PartySceneObject wallObject, Vector3 obstacleSize, bool isFurniture = false)
    {
        if (wallObject._passthroughMesh)
        {
            // Try to get the NavMeshObstacle component, if it doesn't exist, add it
            NavMeshObstacle obstacle = wallObject._passthroughMesh.gameObject.GetComponent<NavMeshObstacle>();
            if (obstacle == null)
                obstacle = wallObject._passthroughMesh.gameObject.AddComponent<NavMeshObstacle>();
            obstacle.carving = true;
            obstacle.shape = NavMeshObstacleShape.Box;

            obstacle.size = obstacleSize;
            obstacle.center = isFurniture ? new Vector3(0,0,-0.5f) : Vector3.zero;
        }
    }
    ...
    public Vector3 GetSimpleFloorPosition()
    {
        ...
        PartySceneObject floor = _roomboxWalls[_roomFloorID];
        if (!floor) return Vector3.zero;
        Vector3 floorPos = new Vector3(PartySceneSingleton.Instance._mainCamera.transform.position.x, floor.transform.position.y, PartySceneSingleton.Instance._mainCamera.transform.position.z);
        return floorPos;
    }
    ...
    {
        Vector3 cameraPos = PartySceneSingleton.Instance._mainCamera.transform.position;
