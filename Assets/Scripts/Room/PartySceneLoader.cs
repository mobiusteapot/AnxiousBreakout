...
public class PartySceneLoader : OVRSceneModelLoader
{
    ...
    {
        PartySceneSingleton.Instance.isGuardianSceneInitialized = true;
    }
    IEnumerator AwaitSceneModel()
    {
        // Subscribe to the SceneModelLoaded event so we can know when the Scene model has been loaded.
        GetComponent<OVRSceneManager>().SceneModelLoadedSuccessfully += SceneModelLoaded;
        const int attemptCount = 5;
        var preamble = $"[{nameof(PartySceneLoader)}]: {nameof(SceneManager.LoadSceneModel)}";
        var attemptsRemaining = attemptCount;
        while (attemptsRemaining > 0)
        {
            if (SceneManager.LoadSceneModel())
            {
                Debug.Log($"[{nameof(PartySceneLoader)}]: Loading scene model...");
                yield break;
            }
    ...
    IEnumerator AwaitSceneCaptureCompletion()
    {
        ...
        Debug.Log($"[{nameof(PartySceneLoader)}]: Requesting Scene Capture.");
        if (SceneManager.RequestSceneCapture())
        {
            ...
            Debug.LogError($"[{nameof(PartySceneLoader)}]: Did not get a response following the completion of Scene Capture.");
        }
        else
        {
            ...
            {
                Debug.Log("Running in editor, trying to run again as no link data has been obtained!");
                //WorldBeyondTutorial.Instance.DisplayMessage(WorldBeyondTutorial.TutorialMessage.ERROR_NO_SCENE_DATA_LINK);
            }

            Debug.LogError($"[{nameof(PartySceneLoader)}]: {nameof(SceneManager.RequestSceneCapture)} returned false.");
            Toast toast = new Toast(new KeyValueList<string, float>()
            {
                { "Fatal error: Scene capture failed!", 100f },
            });
    ...
    protected override void OnNoSceneModelToLoad()
    {
        ...
        {
            Debug.Log("No scene model to load and scene capture has already been launched.");
    ...
    protected override void OnSceneCaptureReturnedWithoutError()
    {
        ...
        Debug.Log($"[{nameof(PartySceneLoader)}]: Scene Capture completed successfully.");
    ...
    {
        sceneCaptureComplete = true;
        Debug.LogError($"[{nameof(PartySceneLoader)}]: Scene Capture failed with an unexpected error.");
